...
from ai_engine.audit_logger import create_log, get_logs, export_logs_json, export_logs_csv, export_logs_pdf
...
app = FastAPI(title="PathFinder AI", version="1.0.0", lifespan=lifespan)
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_credentials=True, allow_methods=["*"], allow_headers=["*"])

active_connections: Dict[str, List[WebSocket]] = {}


async def _connect_ws(student_id: str, websocket: WebSocket) -> None:
    await websocket.accept()
    if student_id not in active_connections:
        active_connections[student_id] = []
    active_connections[student_id].append(websocket)


def _disconnect_ws(student_id: str, websocket: WebSocket) -> None:
    connections = active_connections.get(student_id, [])
    if websocket in connections:
        connections.remove(websocket)
    if not connections and student_id in active_connections:
        del active_connections[student_id]


async def _broadcast_status(student_id: str, payload: dict) -> None:
    connections = list(active_connections.get(student_id, []))
    ...
    for connection in dead_connections:
        _disconnect_ws(student_id, connection)


@app.websocket("/ws/live-status/{student_id}")
async def websocket_live_status(websocket: WebSocket, student_id: str):
    await _connect_ws(student_id, websocket)
    try:
        while True:
            await websocket.receive_text()
    except WebSocketDisconnect:
        _disconnect_ws(student_id, websocket)
    except Exception:
        _disconnect_ws(student_id, websocket)
...
async def apply_to_drive(req: ApplyRequest, db: Session = Depends(get_db)):
    ...
    if not policy_result["passed"]:
        ...
        await _broadcast_status(req.student_id, {
...
@app.get("/audit-logs/export/pdf", tags=["Audit"])
def export_pdf_logs(student_id: Optional[str] = None, drive_id: Optional[str] = None,
                    db: Session = Depends(get_db)):
    logs = get_logs(db, student_id, drive_id, 1000)
    pdf_bytes = export_logs_pdf(logs)
    return Response(
        content=pdf_bytes,
